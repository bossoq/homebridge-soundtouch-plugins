...
export abstract class SoundTouchVolume {
  ...
  protected readonly accessoryWrapper: HomebridgeAccessoryWrapper<SoundTouchDevice> &
    DeviceOnOffListener
  ...
  public constructor(
    ...
    if (this.device.volumeSettings.maxValue < 100)
      this.getVolumeCharacteristic().props.maxValue = Math.min(
        this.device.volumeSettings.maxValue,
        100
      )
  ...
  public async unMuteDevice(unmute: boolean): Promise<boolean> {
    ...
    if (isOn) {
      ...
      if ((unmute && volume.isMuted) || (!unmute && !volume.isMuted)) {
        if (this.device.verbose)
          this.accessoryWrapper.log(
            `[${this.accessoryWrapper.getDisplayName()}] ${
      ...
      if (isOn) return this.accessoryWrapper.deviceDidTurnOn(true)
    }
    return false
  }
  ...
  public async getVolume(): Promise<number> {
    ...
    if (this.device.verbose)
      this.accessoryWrapper.log(
        `[${this.accessoryWrapper.getDisplayName()}] Volume change to ${volume}`
      )
  ...
  public async setVolume(
    ...
    if (secureVolume !== undefined) volume = secureVolume
    if (this.device.verbose)
      this.accessoryWrapper.log(
        `[${this.accessoryWrapper.getDisplayName()}] Volume change to ${volume}`
      )
    if (updateCharacteristic === true) volumeCharacteristic.updateValue(volume)
  ...
  protected volumeChange(change: { newValue: number; oldValue: number }) {
    ...
    if (newValue !== undefined) {
      setTimeout(() => {
        volumeCharacteristic.updateValue(newValue)
      }, 1000)
    }
  }
}
